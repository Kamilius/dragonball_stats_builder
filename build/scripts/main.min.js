...
var LEVEL_CAP = 80, LEVEL = 1, POINTS_PER_LEVEL = 3, TOTAL_POINTS = LEVEL * POINTS_PER_LEVEL, POINTS_DESTRIBUTED = 0, RACE = "majin", GENDER = "male", CHANGE_TIMER, DEFAULT_ATTRIBUTES = {
    majin: {
        male: {
            hea: 8,
            ki: 25,
...
function getAttributeCoordinates(attr, attributeCoordinates) {
    var attrLabelPoint = attributeCoordinates[attr], centerPoint = attributeCoordinates["center"], attrX, attrY, defAttrValue = getAttributeGraphRatio(DEFAULT_ATTRIBUTES[RACE][GENDER][attr]), customAttrValue = getAttributeGraphRatio(CUSTOM_ATTRIBUTES[attr]);
...
function drawAttributes(ctx, attributeCoordinates) {
    var curAttrPoint, heaAttrPoint = getAttributeCoordinates("hea", attributeCoordinates);
...
document.addEventListener("DOMContentLoaded", function() {
    var canvas = document.getElementById("attributes"), ctx = canvas.getContext("2d"), canvasWidth = canvas.width, canvasHeight = canvas.height;
    var attributeCoordinates = {
        center: new Point(canvasWidth / 2, canvasHeight / 2),
        hea: new Point(canvasWidth / 2, 2),
        ki: new Point(canvasWidth - 19, canvasHeight * .25),
        stm: new Point(canvasWidth - 19, canvasHeight * .75),
        atk: new Point(canvasWidth / 2, canvasHeight - 2),
        str: new Point(19, canvasHeight * .75),
        bla: new Point(19, canvasHeight * .25)
    };
    var $raceSelect = document.querySelector(".race-select"), $genderSelect = document.querySelector(".gender-select"), $levelInput = document.querySelector(".level-input"), $attributesList = document.getElementById("attributes-list"), $attributeItems = document.querySelectorAll(".attribute-item"), $pointsToSpend = document.querySelector(".points-to-spend");
    ...
    $attributesList.onkeypress = function(event) {
        var $target = event.target, $attrList = event.currentTarget;
        if ($target.classList.contains("attribute-item")) {
            clearChangeTimer();
            CHANGE_TIMER = setTimeout(function() {
                $attrList.onchange({
